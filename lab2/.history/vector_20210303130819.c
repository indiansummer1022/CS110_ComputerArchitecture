...
vector_t *vector_new() {
    ...
    if (/* YOUR CODE HERE */retval->data == NULL) {
        free(retval);				//Why is this line necessary?-> We have alocate retval successfully, so we need to free it first to avoid memory leak(?)
        allocation_failed();
    }
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    /* YOUR SOLUTION HERE */
    if (loc < v -> size){   // set the value directly
        v->data[loc] = value;
    }
    else{
        int pre_size = v->size;
